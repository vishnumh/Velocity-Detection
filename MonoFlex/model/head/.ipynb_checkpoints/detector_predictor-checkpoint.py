...
class _predictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.reg_headsn = nn.ModuleList()
        
        self.fcsd = nn.Sequential(nn.Conv2d(42, self.head_conv, kernel_size=3, padding=1, bias=False),
                                           InPlaceABN(self.head_conv, momentum=self.bn_momentum, activation=self.abn_activision),
        ...
        for idx, regress_head_key in enumerate(self.regression_head_cfg):
            ...
            self.reg_headsn.append(head_list)
    ...
    def forward(self, features, targets):
        ...
        for i, reg_feature_head in enumerate(self.reg_features):
            ...
            for j, reg_output_head in enumerate(self.reg_headsn[i]):
        ...
        #print("Output regs shape ", output_regs[-2].shape)
        keypointsadd = self.fcsd(output_regs[-2])
        output_regs.append(keypointsadd)
