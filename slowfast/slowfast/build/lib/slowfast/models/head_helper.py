...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        self.projection = nn.Linear(16384, 64, bias=True)
    ...
    def forward(self, inputs):
        ...
        if hasattr(self, "dropout"):
            x = self.dropout(x)
        x = self.projection(x.reshape((x.shape[0], -1))) #16384 
...
class TransformerBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if dropout_rate > 0.0:
            self.dropout = nn.Dropout(dropout_rate)
        self.conf = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 40, bias=True))
        
        self.position = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 40, bias=True))
        
        self.projection = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 40, bias=True))
    ...
    def forward(self, x):
        if hasattr(self, "dropout"):
            x = self.dropout(x)
        c = self.conf(x).view((x.shape[0], 10, 8))
        p = self.position(x).view((x.shape[0], 10, 8))
        v = self.projection(x).view((x.shape[0], 10, 8))
        #x = x.view((x.shape[0], 6, 8))
        
        out = torch.cat((c, p ,v), dim = 2)
