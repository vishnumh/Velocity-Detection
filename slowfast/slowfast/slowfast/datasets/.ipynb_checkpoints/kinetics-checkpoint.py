                ...
                for i,s in enumerate(seq_sampled):
                    ...
                    for j in range(10):
                        ...
                        if j >= l :
                            ...
                            cx = centers[s][j][0]
                            cy = centers[s][j][1]
                            if  j < l and (198 < cx < 423):  # considering detection only if its inside the cropped frame
                                confid_sc.append(1)
                                new_velo.append(np.linalg.norm(velo[s][j]))
                                new_det.append(abs(dets[s][j][3])) #bbox.h, bbox.w, bbox.l, bbox.x, bbox.y, bbox.z, bbox.ry
                            else:
                                confid_sc.append(0)
                                new_velo.append(0)
                                new_det.append(0)
...
#                 print(frame_labels.shape)
        ...
        if self.mode in ["val", "test"]:
            ...
            start =  num_frames_of_folder #int(random.uniform(0, delta))
            seq = list(range(start, start + num_frames * sampling_rate))
...
#             #print(seql)
        ...
        frames = torch.as_tensor(utils.retry_load_images(frames_seq, self._num_retries))
