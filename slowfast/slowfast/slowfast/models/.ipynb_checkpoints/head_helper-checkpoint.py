...
class TransformerBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if dropout_rate > 0.0:
            self.dropout = nn.Dropout(dropout_rate)
        self.conf = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 80, bias=True))
        
        self.position = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 80, bias=True))
        
        self.projection = nn.Sequential(nn.Linear(dim_in, 1024, bias=True), nn.ReLU(), nn.Linear(1024, 80, bias=True))
